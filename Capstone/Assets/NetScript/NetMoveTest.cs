...
public class NetMoveTest : NetworkBehaviour {
    ...
    void MovementUpdate()
    {
        ...
        {
            transform.Translate(Vector3.forward * 1* Time.fixedDeltaTime, player.transform);
        }
        if (Input.GetKey(KeyCode.A))
        {
            transform.Translate(Vector3.right * -1 * Time.fixedDeltaTime, player.transform);
        }
    ...
    void MouseRotation()
    {
        ...
        gun.transform.rotation = localRotationGun;
    ...
    void ShootLocal()
    {
        ...
        GameObject clone;
        clone = Instantiate(projectile, gunPos.position, gunPos.transform.rotation);
